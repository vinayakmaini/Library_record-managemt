...
class Add(Tk):
    def __init__(self):
        ...
        self.title('Add Student to List')
        self.canvas = Canvas(width=500, height=417, bg='blue')
        self.canvas.pack()
        n = StringVar()
        p = StringVar()
        a = StringVar()
#verifying input if entered or not 
        def asi():
            if len(n.get()) < 1:
        ...
        # label and input box
        Label(self, text='Student Details',bg='blue', fg='white', font=('Courier new', 25, 'bold')).pack()
        Label(self, text='Name:',bg='blue', font=('Courier new', 10, 'bold')).place(x=70, y=82)
        Entry(self, textvariable=n, width=30).place(x=200, y=84)
        Label(self, text='Phone Number:',bg='blue', font=('Courier new', 10, 'bold')).place(x=70, y=130)
        Entry(self, textvariable=p, width=30).place(x=200, y=132)
        Label(self, text='Address:',bg='blue', font=('Courier new', 10, 'bold')).place(x=70, y=180)
